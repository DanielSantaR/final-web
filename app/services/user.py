...
from app.schemas.user import AuthUser, CreateUser, PayloadUser, UpdateUser
...
class UserService:
    ...
    async def auth_user(self, user: AuthUser) -> Union[dict, None]:
        user = await self.__user_queries.auth_user(user=user)
        if user:
            return user
        return None

    async def get_all(
        self, *, user: PayloadUser, department_id: int, skip: int, limit: int
    ) -> List:
        user_data = user.dict()
        payload = {
        ...
        users = await self.__user_queries.get_all(
            payload=payload, department_id=department_id, skip=skip, limit=limit
        )
        return users
    ...
    async def update_user(
        ...
        payload = {
            ...
            if (value not in [None, ""] and key not in ["rol", "department", "user"])
        }
        current_update = await self.__user_queries.update(id=user_id, obj_in=payload)
        return current_update

    async def remove_user(self, user_id: int) -> int:
        user_removed_id = await self.__user_queries.delete_user(user_id=user_id)
